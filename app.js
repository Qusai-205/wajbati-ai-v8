...
async function refreshRemote(){try{const [m,s]=await Promise.all([apiGet('menu'),apiGet('settings')]);if(Array.isArray(m.items)&&m.items.length){liveMenu=m.items;WajbatiStore.saveMenu(liveMenu)}if(s.settings){liveSettings=s.settings;WajbatiStore.setSettings(liveSettings)}return true}catch{return false}}
